...
public class SubtopicControllerTesting {
	...
	public void testGetSubtopicsEndpoint() {
	   when().
	       get("http://localhost:9001/subtopics").then().assertThat().
           statusCode(200).
           extract().response();    
	}
	@Test 
	public void testPostSubtopicsEndpoint() {
		...
		String newString = "{\"topicId\": \"5005\",\r\n" + 
			   ...
			   when().
		       post("http://localhost:9001/subtopics?topicId=5005&subtopicName=Microservice").then().assertThat().
	           statusCode(201).
	           extract().response();   
	}
	/**
	...
	public void testGetcheckListOfIds() {
		   when().
	       get("http://localhost:9001/subtopics/verify?ids=1001,1002").then().assertThat().statusCode(200);
	}
	/**
	...
	public void testGetSubtopicsByIds() {
		 when().
	       get("http://localhost:9001/subtopics?ids=1001,1002").then().assertThat().statusCode(200);
	}
}
